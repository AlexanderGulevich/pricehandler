...
public class CommentUtils {
    ...
    public static void setComment(HSSFWorkbook wb, HSSFSheet spreadsheet, Cell cell, PriceItem priceItem) throws IOException, SQLException {
...
//            }
        ...
        try {
            if (bim == null) System.out.println("Ошибка:---"+ priceItem.getName());
            baos = ImgUtils.toOutputStream(bim);

            if (baos == null) {
                Registry.windowFabric.infoWindow("Ошибка:---"+ priceItem);
                System.out.println("Ошибка:---"+ priceItem);
            }
        } catch (IOException e) {
            Registry.windowFabric.infoWindow("Ошибка:---"+ priceItem);
            System.out.println("Ошибка:---"+ priceItem);
